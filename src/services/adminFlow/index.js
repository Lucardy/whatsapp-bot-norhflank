...
export async function isOwnerPhone(phoneNumber, sessionId) {
  try {
    ...
    if (!masterPhone) {
      ...
      try {
        ...
        if (session?.phone_number) {
          ...
          const normalizedPhone = normalizePhoneNumber(phoneNumber.replace('@c.us', ''));
          const normalizedMaster = normalizePhoneNumber(session.phone_number);
    ...
    // Comparar números usando función centralizada de normalización
    const { normalizePhoneNumber } = await import('../../utils/validation/phoneValidator.js');
    const normalizedPhone = normalizePhoneNumber(phoneNumber.replace('@c.us', ''));
    const normalizedMaster = normalizePhoneNumber(masterPhone.replace('@c.us', ''));
...
async function handleAddSessionName(phoneNumber, message, sessionId) {
  ...
  try {
    const { addSessionFromAdmin } = await import('./handlers/addSessionHandler.js');
...
async function handleRemoveSessionSelect(phoneNumber, messageLower, sessionId) {
  ...
  try {
    const { removeSessionFromAdmin } = await import('./handlers/removeSessionHandler.js');
...
async function handleStartSessionSelect(phoneNumber, messageLower, sessionId) {
  ...
  try {
    const { startSessionFromAdmin } = await import('./handlers/startSessionHandler.js');
...
async function handleRegenerateQRSelect(phoneNumber, messageLower, sessionId) {
  ...
  try {
    const { regenerateQRFromAdmin } = await import('./handlers/regenerateQRHandler.js');
